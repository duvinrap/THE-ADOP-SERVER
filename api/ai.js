...
module.exports=async(req,res)=>{
 ...
 try{
  const key=process.env.GEMINI_API_KEY, model=(process.env.GEMINI_MODEL||'gemini-3.8-flash').trim();
  ...
  const rawLanguage=String(body.language||'English').trim().toLowerCase();
  const language=rawLanguage==='ta'||rawLanguage==='tamil'||rawLanguage==='தமிழ்'?'Tamil':rawLanguage==='si'||rawLanguage==='sinhala'||rawLanguage==='සිංහල'?'Sinhala':'English';
  ...
  let response;
  try{response=await fetch('https://generativelanguage.googleapis.com/v1beta/models/'+encodeURIComponent(model)+':generateContent',{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':key},signal:controller.signal,body:JSON.stringify({system_instruction:{parts:[{text:roles[tool]+'\n\nLANGUAGE RULE: Write the entire natural-language response in '+language+'. If Tamil is selected, use clear Tamil script; if Sinhala is selected, use Sinhala script; if English is selected, use English. Keep programming syntax, code identifiers, API names, and required technical keywords unchanged, but explain them in the selected language.'}]},contents:[{role:'user',parts:[{text:prompt+'\n\nReturn a useful structured answer in the selected language. Be original.'}]}]})});}
