...
const ANSWER_LANGS = { en: 'English', ta: 'Tamil (தமிழ்)', si: 'Sinhala (සිංහල)' };
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const language = ANSWER_LANGS[requestedLanguage] ? requestedLanguage : 'en';
    const languageName = ANSWER_LANGS[language];
    ...
    const userText = `${cleanPrompt}\n\nOUTPUT LANGUAGE RULE: Write all natural-language content in ${languageName}. Keep executable code, code syntax, variable/function/class names, engine/API names and required programming keywords in English. Do not switch the explanatory language unless the user explicitly asks.\n\nReturn a useful, structured answer. Be original and do not reproduce copyrighted game text.`;
    ...
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        system_instruction: { parts: [{ text: roles[tool] + `\n\nThe requested natural-language output language is ${languageName}. Follow that language for explanations and headings; keep executable code and programming syntax in English.` }] },
