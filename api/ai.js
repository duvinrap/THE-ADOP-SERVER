...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const langMap = { en: 'English', ta: 'Tamil (தமிழ்)', si: 'Sinhala (සිංහල)' };
    const language = String(body.language || '').trim().toLowerCase();
    const languageInstruction = langMap[language]
      ? `\n\nOUTPUT LANGUAGE RULE: Write all natural-language content in ${langMap[language]}. Keep executable code, code syntax, variable/function/class names, engine/API names and required programming keywords in English.`
      : '';
    if (cleanPrompt.length > 20_000) throw new Error('Prompt is too long.');

    const userText = `${cleanPrompt}${languageInstruction}\n\nReturn a useful, structured answer. Be original and do not reproduce copyrighted game text.`;
    ...
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        system_instruction: { parts: [{ text: roles[tool] + languageInstruction }] },
