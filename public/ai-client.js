...
function getAdopApiBase(){
 try{if(window.AdopAndroid&&typeof window.AdopAndroid.getApiBase==='function'){const b=String(window.AdopAndroid.getApiBase()||'').trim().replace(/\/+$/,'');if(/^https?:\/\//i.test(b)&&!/(localhost|127\.0\.0\.1|10\.0\.2\.2)/i.test(b))return b;}}catch(_){}
