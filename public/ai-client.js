...
// Hosted backend: https://the-adop-server.vercel.app
...
function getAdopApiBase() {
  ...
  try {
    if (
      ...
      if (
        nativeBase &&
        /^https:\/\//i.test(nativeBase) &&
        nativeBase !== 'https://theadop-server.vercel.app'
      ) {
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        language: (localStorage.getItem('adop_ui_lang') || 'en')
      }),
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errMsg || `AI request failed (HTTP ${response.status}).`
      );
    }
